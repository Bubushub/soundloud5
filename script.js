...
function nextTrack1(){
    if(track_index < music_list.length - 2 && isRandom === false){
        track_index += 2;
    }else if(track_index < music_list.length - 2 && isRandom === true){
        let random_index = Number.parseInt(Math.random() * music_list.length);
        track_index = random_index;
    }else{
        track_index = 0;
    ...
    loadTrack(track_index);
    playTrack();
    var selectedSong = document.querySelector('select[name="songs"]').value;
    audio.play(selectedSong);
}
function prevTrack(){
    if(track_index > 0){
        track_index -= 1;
